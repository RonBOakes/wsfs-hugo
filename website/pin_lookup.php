...
EOT;
  ...
  if(($email != '') && ($query->fetch()))
  {
    do
    {
      ...
      var_dump($nameParts);
      print("--!>\n");
      $matches[] = array('first_name'=>$nameParts[0],'last_name'=>$nameParts[1],'member_id'=>$memberId2,'PIN'=>$PIN2,'email'=>$email2);
    }
    while($query->fetch());
  }
  else
...
?>
<P>
  Unable to locate a matching membership.  This could mean one of several things:
  <UL>
    <LI>
        There is not a valid membership to MidAmeriCon II that matches this information
    </LI>
    <LI>
        The matching membership does not have an email address
    </LI>
  </UL>
</P>
<P>
  Please contact the Hugo Award PIN administrator at <A HREF="mailto:hugopin@worldcon76.org">hugopin@worldcon76.org</A> to
  enquire further.
</P>
<?PHP
  }
...
?>
<P>
    An email has been sent to the email address on record for the selected member.  Please check your email for a message from
    hugopin@worldcon76.org which will contain your 2018 Hugo Award, Award for Best Young Adult Book, and John W. Campbell Award 
    nominating information.  This match was made exclusively by name, so it is possible that the matching record belongs to a 
    different person, and the email was sent to them instead.
</P>
<?PHP
    }
    else
...
?>
<P>
  <B>All fields are required for a successful lookup.</B>
  Your Hugo PIN and membership number will be emailed to you.
  If you do not received your PIN, you may email
  <A HREF="mailto:hugopin@worldcon76.org">hugopin@worldcon76.org</A>
  for assistance.
</P>
